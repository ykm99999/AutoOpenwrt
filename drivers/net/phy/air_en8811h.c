...
{
	int ret;
#if AIR_UBOOT_REVISION > 0x201807
...
static int en8811h_led_init(struct phy_device *phydev)
{
    ...
    for(led_id = 0; led_id < EN8811H_LED_COUNT; led_id++)
    {
        ...
        if (led_cfg[led_id].en == LED_ENABLE)
        {
            ...
            ret = airoha_led_set_usr_def(phydev, led_id, led_cfg[led_id].pol,
                            led_cfg[led_id].on_cfg, led_cfg[led_id].blk_cfg);
            if (ret != 0) {
...
static int air_config_clk_out(struct phy_device *phydev)
{
    u32 pbus_value;
    int ret;
#if AIR_UBOOT_REVISION > 0x201807
...
static int en8811h_config(struct phy_device *phydev)
{
    ...
    /* Serdes polarity */
    pbus_value = air_buckpbus_reg_read(phydev, EN8811H_POLARITY);
#if AIR_UBOOT_REVISION > 0x201807
    ...
    if (ofnode_read_bool(node, "airoha,pnswap-tx"))
        ...
        pbus_value |=  EN8811H_POLARITY_TX_NORMAL;
#else
    pbus_value &= ~EN8811H_POLARITY_MASK;
    pbus_value |= (EN8811H_RX_POLARITY_NORMAL | EN8811H_TX_POLARITY_NORMAL);
#endif
    ...
    pbus_value = air_buckpbus_reg_read(phydev, EN8811H_FW_VERSION);
    printf("MD32 FW Version(0x3b3c) : %08x\n", pbus_value);

    ret = air_config_clk_out(phydev);
    if (ret < 0)
        return ret;
...
static int en8811h_startup(struct phy_device *phydev)
{
    ...
    phydev->supported = (SUPPORTED_100baseT_Full | SUPPORTED_1000baseT_Full | SUPPORTED_2500baseX_Full);
